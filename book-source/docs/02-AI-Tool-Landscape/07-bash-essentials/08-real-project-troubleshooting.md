...
## Exercise 1: Interpret Error Messages

For each error, identify the type and what it means:
...
```
- **Type**: Import Error
- **Meaning**: The python-dotenv package isn't installed
- **First step**: `pip install python-dotenv`
...
**Error 2**:
- **Type**: File Error
- **Meaning**: The `.env` configuration file doesn't exist
- **First step**: Create it with `touch .env` or `cat > .env << 'EOF'...`

**Error 3**:
- **Type**: Connection Error
- **Meaning**: Can't reach the API server (network issue, wrong URL, or service is down)
- **First step**: Check internet connection, verify the URL, or contact API support

---

## Exercise 2: Real Troubleshooting Scenario

You're setting up a Node.js project and encounter this sequence:

1. First run: `Error: Cannot find module 'express'`
2. After `npm install express`: `Error: Cannot find module 'dotenv'`
3. After installing dotenv: `Error: listen EADDRINUSE: address already in use :::3000`

**For each error, write what the issue is and how to fix it**:
...
1. **Error 1**: Dependencies not installed yet. Fix: Run `npm install` (or `npm install express`)
2. **Error 2**: Another dependency missing. Fix: Run `npm install dotenv` (or `npm install` to install all)
3. **Error 3**: Another process is already using port 3000. Fix: Change port in code, or kill the existing process with `lsof -i :3000` then `kill <PID>`

---

## Exercise 3: Build Your Own Troubleshooting Plan

You have a Python project with a database. Write what errors you EXPECT to encounter and how you'd troubleshoot them:

**Expected Errors**:
1. What could go wrong with database connection?
2. What could go wrong with missing files?
3. What could go wrong with permissions?

**Your answers** (for each, describe error and first debugging step):
...
1. **Database connection error**: `ConnectionError: connect call failed`. First step: Verify database is running, check host/port/credentials in .env
2. **Missing file error**: `FileNotFoundError: [Errno 2]`. First step: Check file path, verify file exists, check working directory
3. **Permission error**: `PermissionError: [Errno 13] Permission denied`. First step: Check file permissions (`ls -la`), may need `sudo` or `chmod`
...
### Safety and Learning Note

This lesson is the capstone because it's where all your learning integrates. Real work involves errors—that's completely normal and expected. Professional developers spend 30-50% of their time debugging. What matters is:
- **Reading error messages** (they're helpful)
- **Asking for explanation** (you don't have to figure it out alone)
- **Following logical steps** (troubleshooting is a process)
- **Learning from each fix** (building knowledge over time)

You've learned enough to supervise complex operations with your AI. Errors teach you more than success does. Welcome them as learning opportunities.

---

## Integrated Concepts Checklist

By this lesson, you've learned and applied:

- [ ] **Navigation** (cd, pwd, ls) to move through folders
- [ ] **Safety pattern** (Ask → Explain → Understand → Verify → Execute)
- [ ] **File operations** (cp, mv, rm) with backup strategies
- [ ] **Configuration** (environment variables, .env files, no hardcoding)
- [ ] **Packages** (pip, npm, installation, dependencies)
- [ ] **Pipelines** (ls | grep | wc, data flow)
- [ ] **Error interpretation** (reading and understanding error messages)
- [ ] **Troubleshooting** (systematic debugging with AI)
